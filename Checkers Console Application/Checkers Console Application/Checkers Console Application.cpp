...
public:
    ...
    int GetPlayTileIndex(PlayTile* playTile) {
        int i = playTile->GetRowNumber();
        int j = playTile->GetColumnNumber();
        for (int index = 0; index < 32; index++) {
            if (i == playableTiles[index].GetRowNumber() && j == playableTiles[index].GetColumnNumber()) return index;
        }
        return -1;
    }
    ...
    void FindMoves(PlayTile* currPlayTile) {
        if (isMyPiece(currPlayTile)) {
            ...
            for (int i = 0; i < currPlayTile->adjacentPlayTiles.size(); i++) {
                ...
                if(adjTile->GetOccupance()) cout << gameBoard->GetPlayTileIndex(currPlayTile->adjacentPlayTiles[i]) << " ";
            } 
        }
    }
