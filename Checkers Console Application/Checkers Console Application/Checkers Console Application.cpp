...
int main() //alot of the game logic might not be the best. I wanted to focus on the cohesion between the main classes.
{
    ...
    CheckersPlayer playerOne, playerTwo;
    string playerName;
    string input;
    string fromString, toString;
    gameBoard.BoardSetup(boardSize);//board starts out as empty.
    gameBoard.DrawBoard();
    ...
    playerOne = CheckersPlayer(tolower(playerOneColor), 1, &gameBoard, playerName);
    ...
    playerTwo = CheckersPlayer(tolower(playerTwoColor), -1, &gameBoard, "The Computer");
    ...
    while (!donePlaying) {
        /*if (playerOne.GetPieceCount() == 0 || playerTwo.GetPieceCount() == 0) {
            string winnerName = (playerOne.GetPieceCount()==0) ? playerTwo.GetName() : playerOne.GetName();
